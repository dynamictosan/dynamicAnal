...
sys = add_body(sys, "slider", [-0.5, 0], deg2rad(15));
sys = add_body(sys, "link", [-0.7, 0.05], deg2rad(15));
...
sys = add_joint_translation(sys, "link", "slider", [0.2; 0], [0.05; 0], [0.0, 0.0]);

sys = add_joint_revolute(sys, "slider", "ground", [0.0; 0]);
...
sys = add_force_torsional_spring(sys, "ground", "crank", kn, cn, 0);

f = 400;
% end force
f_t_1 = [f; 0];
f_t_2 = @(t) [0; -exp(t - 2) * f];
sys = add_force_single(sys, "slider", f_t_1, [0; 0.0]); % add force interface: sys, body_name, forcevector, location in local body frame
...
pidx = 4;
plot(T, Q(:,pidx), ...
    T(1:end-1), (Q( 2:end, pidx)-Q(1:end-1,pidx))/0.001, '--')
%% SOLVER fsolve
